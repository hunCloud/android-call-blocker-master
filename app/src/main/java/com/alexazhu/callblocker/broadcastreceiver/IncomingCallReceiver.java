    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (TelephonyManager.EXTRA_STATE_RINGING.equals(newState)) {
            ...
            AsyncExecutorUtil.getInstance().getExecutor().execute(() -> {
                Log.i(LOG_TAG, "Zoooooo");
                Optional<BlockedNumber> match = blockedNumberDao.getAll().stream().filter(blockedNumber -> blockedNumber.
                        getRegex().matcher(phoneNumber).find()).findAny();
                if (!match.isPresent()) {
                    Log.i(LOG_TAG, match.toString()+"No blocked number matched");
                    return;
                }
                Log.i(LOG_TAG, String.format(match.toString()+"B54545locked number matched: %s", match.get().toFormattedString()));
